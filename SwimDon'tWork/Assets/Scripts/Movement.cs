...
public class Movement : MonoBehaviour {
    ...
    bool CheckInSafetyZone()
    {
        ...
        if (Mathf.Abs(transform.position.x-referencePoint.x) < radius * 0.75f && Mathf.Abs(transform.position.z - referencePoint.z) < radius*0.75f && Mathf.Abs(transform.position.y - referencePoint .y) < radius * 2)
        {
            return true;
        }
        return false;
